...
`,e=e.substring(p.length+1),l=!0),!l){const T=new RegExp(`^ {0,${Math.min(3,E-1)}}(?:[*+-]|\\d{1,9}[.)])((?:[ 	][^\\n]*)?(?:\\n|$))`),x=new RegExp(`^ {0,${Math.min(3,E-1)}}((?:- *){3,}|(?:_ *){3,}|(?:\\* *){3,})(?:\\n+|$)`),w=new RegExp(`^ {0,${Math.min(3,E-1)}}(?:\`\`\`|~~~)`),_=new RegExp(`^ {0,${Math.min(3,E-1)}}#`);for(;e;){const $=e.split(`
`,1)[0];let N;if(p=$,this.options.pedantic?(p=p.replace(/^ {1,4}(?=( {4})*[^ ])/g,"  "),N=p):N=p.replace(/\t/g,"    "),w.test(p)||_.test(p)||T.test(p)||x.test(p))break;if(N.search(/[^ ]/)>=E||!p.trim())c+=`
